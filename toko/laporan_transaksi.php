...
$sql = "SELECT * FROM `data_transaksi` LEFT JOIN `data_pelanggan` ON data_transaksi.id_pelanggan = data_pelanggan.id_pelanggan WHERE ((data_transaksi.id_toko = '$_SESSION[id]') AND (data_transaksi.tgl_transaksi >= '$tanggal_awal 00:00:00') AND (data_transaksi.tgl_transaksi <= '$tanggal_akhir 23:59:00')) ORDER BY data_transaksi.id_transaksi DESC";
		...
		<tbody>
			<?php while ($transaksi = mysqli_fetch_array($transaksiAll, MYSQLI_BOTH)) : ?>
				...
				<tr>
					...
					<th><?= showRating($transaksi['rating'], 11) ?></th>
				</tr>
				<?php $i++; ?>
			<?php endwhile ?>
		</tbody>
