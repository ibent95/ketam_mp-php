<?php
	...
	$transaksiAll = getTransaksiSubJoinLimitAll($page, 10, 'tunggu');
								...
								<?php else : ?>
									<?php while ($data = mysqli_fetch_array($transaksiAll, MYSQLI_BOTH)) : ?>
										<tr>
											...
											<td>
												<!--<?php // if ($data['status_transaksi'] == 'tunggu' and $data['id_toko'] < 1) :  ?>
												...
												<?php // elseif ($data['status_transaksi'] == 'proses' and $data['diantarkan'] == "ya" and $data['id_toko'] < 1) :  ?>
													<a class="btn btn-primary btn-sm" href="?content=data_transaksi_toko_form&action=persetujuan&id=<?php // echo $data['id_transaksi']; ?>">
														<i class="fas fa-user-plus"></i>
														Tentukan Toko
													</a>
												<?php // elseif ($data['status_transaksi'] == 'proses' and $data['status_pengembalian'] === 'belum' and $data['diantarkan'] === 'tidak' and ($data['id_toko'] < 1 or $data['id_toko'] == NULL)) : ?>
													<a class="btn btn-success btn-sm" <?php // echo "href=\"?content=data_transaksi_detail&action=konfirmasi_pengembalian&id=" . $data['id_transaksi'] . "\""; ?>>
														<i class="fas fa-check"></i>
														Konfirmasi Pengembalian
													</a>
												<?php // elseif ($data['status_transaksi'] == 'proses' and $data['status_pengembalian'] === 'sudah' and $data['diantarkan'] == "tidak" and ($data['id_toko'] < 1 or $data['id_toko'] == NULL)) : ?>
